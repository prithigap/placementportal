...
// Trigger Delete Modal with the selected company ID
const delCompany = (currentAnn) => {
  if (currentAnn && (currentAnn.id || currentAnn._id)) {
      const companyId = currentAnn.id || currentAnn._id;
      setCurrent({ did: companyId });
      console.log("Setting current company ID for deletion:", companyId); // Debugging log
      openRef.current.click(); // Opens confirmation modal
  } else {
      console.error("Company ID is undefined for deletion.");
  }
};

// Confirm deletion and call deleteCompany with ID
const handleDelete = () => {
  if (current.did) {
      console.log("Deleting company with ID:", current.did); // Debugging log
      deleteCompany(current.did); // Calls deleteCompany with the ID
      refdel.current.click(); // Closes confirmation modal
      ...
      console.error("No company ID found to delete."); // Debugging log
  }
};
